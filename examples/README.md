...
| 56 | [Lock Downgrade](56-lock-downgrade/) | `LockDowngradeDetector` | Write lock released before read lock acquired, so another thread can write in the gap. Reported when a writer is observed inside the gap; the shape alone is also correct code (#355) | 🔴 Critical |
...
| 64 | [Phaser Misuse](64-phaser-misuse/) | `PhaserDetector` | Fewer parties registered than threads that arrive — a phase that never advances | 🟡 High |
...
| 144 | [Virtual Thread Monitor Serialization](144-vthread-monitor-serialization/) | `VirtualThreadMonitorSerializationDetector` | JEP 491 removed the pinning and left the bottleneck — `synchronized` still admits one thread at a time, and nothing bounds how many arrive | 🟡 Medium |
