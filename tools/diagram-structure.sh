...
# sorted multiset of <title> elements (node identity plus kind, e.g. "ConcurrencyRunner [CLASS]").
#
# Why structure and not bytes: code-karta's directory walk follows filesystem order, so the
# same sources render with nodes in different positions on NTFS and ext4, and member labels
# truncate at a font-metric-dependent width. Both make byte and label comparison
