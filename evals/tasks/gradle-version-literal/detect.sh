...
set -uo pipefail
if git diff -- build.gradle.kts async-test-lib/build.gradle.kts async-test-agent/build.gradle.kts async-test-analysis/build.gradle.kts \
   | grep -E '^\+' | grep -v '^\+\+\+' | grep -Eq '[0-9]+\.[0-9]+\.[0-9]+'; then
  echo "violation: a version literal was added to a Gradle build file"
  exit 1
fi
if git diff -- pom.xml | grep -Eq '^\+.*<archunit\.version>'; then
