...
| PMD | `pmd.version`, `maven-pmd-plugin.version` | Static analysis. Pinned to a version whose `LooseCoupling` rule is JDK-26-clean — see [quality-gates/platforms.md](quality-gates/platforms.md#build-with-jdk-21-25-or-26) for the measurement. |
...
| `org.apache.groovy:groovy` | `groovy.version` | Apache-2.0, and every dependency it declares is optional, so it adds one jar. Agent-pair lane only (#545). `StringGroovyMethods.getCount(Matcher)` calls `reset()` and `find()` on the `Matcher` its caller passes, in Groovy's own class file, which makes it the one public API across the surveyed libraries that lets `SHARED_MATCHER` be measured on a library call site: every corpus library before it builds a `Matcher` per call. Groovy states no thread-safety contract of its own; the ground truth is `java.util.regex.Matcher`'s ("not safe for use by multiple concurrent threads"), the same kind of evidence as the commons-lang3 `FormattableUtils` pair. |
