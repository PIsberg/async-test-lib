...
| StampedLockDetector | fires (a failed `validate()` with no fallback; a write stamp one thread took and never released) | silent (the `validate()`-then-`readLock()` fallback; the same write released in a `finally`) | leaks since #588 need two facts: an acquisition no recorded unlock matched, and the lock still held at analysis; matching is per thread and lock instance, not per name |
...
| ConditionVariableDetector | fires (the producer signals `notFull`, the consumer stays parked on `notEmpty` at analysis); with the lock registered, fires from the lock's wait queue on the same parked consumer | silent when the producer signals `notEmpty`, including a second signal made into an empty condition; with the lock registered, silent on an await the body recorded but never parked in | per-await pairing since #583: a timed-out await and a signal with nobody waiting are not findings. Since #592 a condition registered with its lock has stuck waiters read from `getWaitQueueLength`; registered alone, a stuck waiter is still the recording. A worker interrupted at the round timeout has left the queue before analysis, so the lock cannot count it |
...
| WakeupDetector | fires (an `if`-guarded wait returns before anyone notifies and the consumer goes on) | silent when a `while` loop waits again after the same return, including a notify made with nobody waiting | re-check observed since #590: an unsignalled return is a finding only when the same thread does not wait again; the recording API cannot see a timeout, so a timed wait whose caller gives up by design would fire |
