...
| Allocation budget | `RunnerAllocationBudgetTest` (`-P e2e`, every CI leg) | every CI build | one all-detector `@AsyncTest` run allocates more than 80,000 bytes per body execution (3.0x the 25,985 to 26,599 measured on 2026-08-15), or a body that records through the `SelfGuard`, `HappensBefore`, `RaceConditionDetector` and agent field-event paths allocates more than 110,000 bytes per execution beyond the empty body (1.21x the 84,297 to 91,227 measured on JDK 21, 24 and 26 on 2026-09-26, #752). Re-derive either the same way, red first |
