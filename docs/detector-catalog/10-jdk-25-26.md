...
* **Severity**: `HIGH` (missing combiner) · `HIGH` (one state object shared across segments)
* **Description**: On a parallel stream the runtime splits the input, runs the integrator on
  ...
  results. The detector confirms multi-thread execution and judges whether it was safe. A
  gatherer that declares a combiner is judged only on state identity: its integrator running on
  several threads is the parallel contract working, and it fires only when one state object is
  integrated on two threads (pass the state to `recordIntegrate(name, state, thread)`). A label
  registered twice with different shapes draws no missing-combiner verdict.
* **Buggy Code**:
  ```java
...
* **Detect**:
  ...
  d.registerGatherer("merged", /*hasCombiner*/ true, /*parallel*/ true);
  // integrator calls d.recordIntegrate("merged", state, Thread.currentThread()); silent while
  // each segment integrates its own state, fires when one state reaches two threads
  ```
