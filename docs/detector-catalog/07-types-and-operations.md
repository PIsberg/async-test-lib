...
* **Severity**: `HIGH`
* **Description**: Detects lambdas/`Runnable`/`Callable` instances that capture a mutable container (an array, an outer field, or an Atomic used via get+set) and are subsequently executed concurrently. The JVM's effectively-final rule only covers the captured *reference*, not a mutable container's contents, so shared execution introduces a data race. Mutation of a captured object that is thread-safe by type (named through `recordCapturedMutation(lambda, name, state, thread)`: `java.util.concurrent` and its `atomic` package, or a `Collections.synchronizedXxx` wrapper) is not reported, nor is mutation that one lock covered every time: the captured object's own monitor, a lock declared with `AsyncTestContext.holdingLock(...)`, or one the agent wove. The lock is judged per captured object, so a lambda whose two named captures are each guarded by a different lock is not reported; a capture guarded by one lock on one thread and another lock on a second thread still is. The lock is judged per object, not per name, so one object recorded under two names is one capture. A mutation recorded without the captured object may be of any capture, so a lambda with one is judged as one capture, all its mutations together: two unnamed captures each under its own lock are still reported, and so is a lambda whose named captures each hold their own lock once it also records an unnamed mutation; name every object to have each judged alone. A thread that only reads the capture counts once the read is recorded with `recordCapturedRead(lambda, state, thread)`, probed for locks on the reading thread: one writer beside unguarded readers is reported, a writer and readers all under one lock are not, and reads with no mutation are not. The mutation has to be in the same round as the reads: unguarded readers in one round and a writer alone in another never overlapped and are not reported. `recordExecution` names no capture and does not count as a read.
* **Buggy Code**:
  ```java
