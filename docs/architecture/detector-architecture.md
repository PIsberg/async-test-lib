...
- **Phase 9** — formerly repository & environment state: `UncommittedChangesDetector` (removed after 1.7.2 — a git-status environment check, not a concurrency property)
...
- **Phase 16** — JDK 25/26 preview-era: `StableValueMisuseDetector` (read-before-set / double-set / reentrant `orElseSet`), `StructuredTaskScopeMisuseDetector` (fork-after-join / result-before-join / owner-confinement / missing join, plus JDK 26 join-timeout events since 1.7.0), `GathererConcurrencyMisuseDetector` (stateful parallel gatherer without a combiner)
