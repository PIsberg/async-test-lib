...
The build is a three-module Maven reactor. The repository root holds only the parent POM and shared
configuration — there are no sources at the root any more.

| Module | Artifact | What it holds |
|--------|----------|---------------|
| `async-test-lib/` | `async-test-lib` (unchanged coordinate) | annotations, config, runner, extension, the ~129 detectors, SPI, reporting, benchmark, telemetry |
...
| `async-test-analysis/` | `async-test-analysis` | `StaticPinningScanner`. ASM only; depends on nothing else in the project |

`ArchitectureTest` pins these boundaries: nothing may depend on the agent or the analysis module,
and byte-buddy / asm may not leak out of them. See [MODULARIZATION.md](MODULARIZATION.md) for why
the split stops where it does — the detector set cannot move until the dual-registry question in
[ROADMAP_V2.md](ROADMAP_V2.md) Train 3 is settled.

Each module has its own `CLAUDE.md` and `.claude/rules/`, generated by vibetags from that module's
own annotations. When editing inside a module, that file is the authoritative one; the root
