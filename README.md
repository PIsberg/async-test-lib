...
- **Optional agent** — `async-test-agent` rewrites field accesses, collection and lock calls, shared JDK objects, coordination primitives, `Object.wait` and `notify`, `Thread.sleep`, `System.gc`, `Thread.start` and `Thread.setDaemon` with Byte Buddy, so **23 of them can see code you did not modify** instead of 3. Being able to see is not the same as firing: on 82 third-party subjects two of the 21 exposed at the time produced every finding, and the other nineteen were correctly silent because nothing in that corpus writes the idiom they model. Not needed for default use, and the core artifact does not depend on Byte Buddy ([docs/AGENT.md](docs/AGENT.md)).
...
silent, and the other 15 carry a written reason. 60 of those pairs are what let their detectors
...
nobody here compiled; `EXPLICIT_GC` is refused a pair in every lane, `MISSED_SIGNAL` has a JDK
pair only, because no corpus library waits behind an `if`, and `DAEMON_THREAD_HYGIENE` has no
agent pair yet (#736). Their
...
all. With the agent attached that becomes 23, because the woven streams carry what those
detectors need without a line of instrumentation. The remaining 123 observe what the test body
