...
**JUnit 5 concurrency stress testing — one annotation, 121 detectors**
...
- **121 detectors** — deadlocks, race conditions, virtual-thread pinning, lifecycle bugs, misused JDK types, JDBC sharing, MessageDigest/SecureRandom/Cipher integrity, and more — all on by default (`detectAll = true`), or pick a `Preset` for a curated subset.
...
1. **Add the dependency** to `build.gradle.kts`:
   ```kotlin
   testImplementation("se.deversity.async-test-lib:async-test-lib:1.7.0-RC3")
   ```
